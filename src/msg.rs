...
pub mod experiment {
    ...
    pub fn add_extras(graph: &mut BTreeMap<RemoteId, Vec<RemoteId>>, n_extras: u16) {
        let max_remote: RemoteId = *graph.keys().max().unwrap_or(&0);
        for id in (max_remote + 1)..(n_extras + max_remote + 1) {
